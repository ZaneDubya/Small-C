...
int seqdata[HIGH_SEQ + 1][SEQLEN] = {
    // 00: ADD12 MOVE21 (ADD pr to sr then MOVE sr to pr) ->
    //     Replacing ADD12 with ADD21, and advance snext by 1 to skip the now redundant MOVE21
    { 0,ADD12,MOVE21,0,                      go|p1,ADD21,0 },
    // 01: ADD1n (adds constant n to pr) ->
    //     if n < -2, do nothing (no point in replacing 1x3 byte op with 3+x1 byte ops)
    //     if n < 0, rDEC1 n times (negating n)
    //     if n < 3, rINC1 n times
    { 0,ADD1n,0,                             ifl|m2,0,ifl|0,rDEC1,neg,0,ifl|p3,rINC1,0,0 },
    // 02: ADD2n (adds constant n to sr) ->
    //     if n < -2, do nothing (no point in replacing 1x3 byte op with 3+x1 byte ops)
    //     if n < 0, rDEC2 n times (negating n)
    //     if n < 3, rINC2 n times
    { 0,ADD2n,0,                             ifl|m2,0,ifl|0,rDEC2,neg,0,ifl|p3,rINC2,0,0 },
    // 03: char *ptr; x = (*ptr)--; ->
    //     Getb1p [rDEC1 PUTbp1 rINC1] (byte pr = mem[sr], pr -= n, mem[sr] = byte pr, pr += n) ->
    //     if n == 1, replace [] with DECbp (byte [sr]--) and advance snext by 2
    //     else,      replace [] with SUBbpn (byte [sr] -= n) and advance snext by 2
    //     * This code sequence is only generated by pointer dereference and post-decrement
    { 0,rDEC1,PUTbp1,rINC1,0,               go|p2,ife|p1,DECbp,0,SUBbpn,0 },
    // 04: int *ptr; x = (*ptr)--; ->
    //     GETw1p [rDEC1 PUTwp1 rINC1] (word pr = mem[sr], pr -= n, mem[sr] = word pr, pr += n) ->
    //     if n == 1, replace [] with DECwp (word [sr]--) and advance snext by 2
    //     else,      replace [] with SUBwpn (word [sr] -= n) and advance snext by 2
    //     * This code sequence is only generated by pointer dereference and post-decrement
    { 0,rDEC1,PUTwp1,rINC1,0,               go|p2,ife|p1,DECwp,0,SUBwpn,0 },
    // 05: char g; x = g--; (g is a global/static char) ->
    //     GETb1m [rDEC1 PUTbm1 rINC1] (byte pr = g, pr -= n, g = byte pr, pr += n) ->
    //     replace [] with SUB_m_ COMMAn (SUB BYTE PTR g, n)
    //     * This code sequence is only generated by direct global/static variable post-decrement
    { 0,rDEC1,PUTbm1,rINC1,0,               go|p1,SUB_m_,go|p1,COMMAn,go|m1,0 },
    // 06: int g; x = g--; (g is a global/static int) ->
    //     GETw1m [rDEC1 PUTwm1 rINC1] (word pr = g, pr -= n, g = word pr, pr += n) ->
    //     replace [] with SUB_m_ COMMAn (SUB WORD PTR g, n)
    //     * This code sequence is only generated by direct global/static variable post-decrement
    { 0,rDEC1,PUTwm1,rINC1,0,               go|p1,SUB_m_,go|p1,COMMAn,go|m1,0 },
